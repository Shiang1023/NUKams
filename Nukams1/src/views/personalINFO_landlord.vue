...
export default {
  ...
  methods: {
    getUserInfo(id) {
      const path = `http://127.0.0.1:5000/api/landlord/landlord_information/${id}`;
      console.log(path);
      axios.get(path)
        .then((res) => {
          this.user = res.data.landlord_information[0];
    ...
    updateUserInfo() {
      const path = `http://127.0.0.1:5000/api/landlord/landlord_modify_information/${this.user.id},${this.user.name},${this.user.email},${this.user.phone}`;
