        ...
        <table v-if="Object.keys(students).length > 0">
          ...
          <tbody>
            <tr v-for="student in filteredStudents" :key="student.id">
              <td><router-link :to="{ name: 'fillRecord', params: { id: student.student_id, name: student.name }}">{{ student.student_id }}</router-link></td>
              <td><router-link :to="{ name: 'fillRecord', params: { id: student.student_id, name: student.name }}">{{ student.name }}</router-link></td>
            </tr>
          </tbody>
        </table>
