<template>
  <div class="wrapper">
    ...
    <main>
      <div v-for="comment in comments" :key="comment.comment_id" class="button-container">
        <div class="button">
...
export default {
  ...
  methods: {
    getComments() {
      const path = 'http://127.0.0.1:5000/api/administrator/get_all_comment';
      axios.get(path)
        .then((res) => {
          this.comments = res.data.post_info;
        })
        .catch((error) => {
          console.error(error);
        });
