...
export default {
  ...
  methods: {
    getUserInfo(id) {
      const path = `http://127.0.0.1:5000/api/administrator/administrator_information/${id}`;
      console.log(path);
      axios.get(path)
        .then((res) => {
          this.user = res.data.administrator_information[0];
    ...
    updateUserInfo() {
      const path = `http://127.0.0.1:5000/api/administrator/administrator_modify_information/${this.user.id},${this.user.name},${this.user.email},${this.user.phone}`;
