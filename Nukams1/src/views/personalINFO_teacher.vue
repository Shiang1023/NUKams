  ...
  <div id="app">
    <div class="user-info">
      ...
      <div v-if="!isEditing">
        ...
        <p><span>教職:</span> {{ user.level }}</p>
        <p><span>連絡電話:</span> {{ user.phone }}</p>
        <p><span>辦公室地址:</span> {{ user.office_address }}</p>
        <p><span>辦公室電話:</span> {{ user.office_phone }}</p>
      ...
      <form v-else @submit.prevent="updateUserInfo">
        ...
        <label>辦公室地址</label>
        <input type="text" v-model="user.office_address">

        <label>辦公室電話</label>
        <input type="text" v-model="user.office_phone">
...
export default {
  ...
  methods: {
    getUserInfo(id) {
      const path = `http://127.0.0.1:5000/api/teacher/teacher_information/${id}`;
      console.log(path);
      axios.get(path)
        .then((res) => {
          this.user = res.data.teacher_information[0];
    ...
    updateUserInfo() {
      const path = `http://127.0.0.1:5000/api/teacher/teacher_modify_information/${this.user.id},${this.user.name},${this.user.email},${this.user.level},${this.user.phone},${this.user.office_address},${this.user.office_phone}`;
